...
def test_legacy_remains_exclusive(monkeypatch):
    calls = []
    monkeypatch.setattr(direct, "check_idle_gpus", lambda devices: calls.append(devices))
...
def test_shared_gpu_rejects_insufficient_headroom(monkeypatch, free, utilization):
    monkeypatch.setattr(direct.subprocess, "check_output", lambda *_a, **_k: f"{GPU}, {free}, {utilization}\n")
    with pytest.raises(ResourceBusyError):
        direct.check_gpu_admission((GPU,), shared_profile())


@pytest.mark.parametrize("text", ["", f"{GPU}, N/A, 0\n", f"{GPU}, -1, 0\n", f"{GPU}, 60000, 101\n", f"{GPU}, 60000, 0\n{GPU}, 60000, 0\n"])
def test_shared_gpu_bad_inventory_fails_closed(monkeypatch, text):
    ...
    with pytest.raises(ValueError, match="GPU|invalid literal"):
        direct.check_gpu_admission((GPU,), shared_profile())
