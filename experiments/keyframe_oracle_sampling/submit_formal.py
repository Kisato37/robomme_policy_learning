...
def build_submission(
    ...
    *,
    ...
    if record_path.exists():
        raise FileExistsError(f"Refusing duplicate formal submission: {record_path}")
    ...
    row_spec = f"0-{len(rows) - 1}" if attempt_id == 0 else ",".join(str(row_id) for row_id in selected_rows)
...
def main() -> None:
    ...
    atomic_write_json(submission_path(args.run_root, args.attempt_id), record)
    print(job_id)
